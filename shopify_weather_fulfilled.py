...
def process_orders_csv_fallback(csv_file="orders_export (4).csv"):
    ...
    try:
        with open(csv_file, 'r') as file:
            ...
            # Track fulfillment status for each order
            order_fulfillment_status = {}
            
            # First pass: collect all items and quantities for each order and check fulfillment status
            for row in reader:
                ...
                if fulfillment_status and order_id not in order_fulfillment_status:
                    order_fulfillment_status[order_id] = fulfillment_status
            ...
            # Second pass: process shipping decisions for unfulfilled orders only
            for row in reader:
                # Extract order ID (removing the # symbol if present)
                order_id = row.get('Name', '')
                if order_id.startswith('#'):
                ...
                # Skip if order is already fulfilled
                if order_id in order_fulfillment_status and order_fulfillment_status[order_id].lower() == 'fulfilled':
                    print(f"Order {order_id} is already fulfilled, skipping.")
...
if __name__ == "__main__":
    ...
    print("Processing orders from CSV file...")
    shipping_decisions = process_orders_csv_fallback()
